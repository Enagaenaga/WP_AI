...
# Write with CP932 (Shift-JIS) encoding
target_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "start_wp-ai.bat")
# Adjust path because this script will be in wp-ai or root? 
# Let's assume we run this from the root or check where we are.
# Actually, let's just use the absolute path provided in the prompt context if possible, 
# but for safety, I will write to the specific path.

target_path = r"c:\Users\shima\OneDrive\ドキュメント\GitHub\WP-AI\start_wp-ai.bat"

try:
    with open(target_path, "w", encoding="cp932") as f:
        f.write(batch_content)
    print(f"Successfully wrote {target_path} in CP932")
